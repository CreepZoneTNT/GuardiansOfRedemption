...
public class RedemptionGuardian : ModPlayer
{
    ...
    public void SpawnDragonLeadStandardFire(NPC target, int damage) {
        DustHelper.DrawCircle(target.Center, DustID.IceTorch, 1, 4, 4, nogravity: true);
        for (int i = 0; i < 3; i++) {
            Vector2 sourcePos = target.Center - Vector2.UnitY.RotatedBy(Main.rand.NextFloat(-MathHelper.Pi/3, MathHelper.Pi/3)) * Main.rand.NextFloat(12f, 24f);
            Vector2 velocity = sourcePos.DirectionTo(target.Center) * 8f;
            Projectile.NewProjectileDirect(
                target.GetSource_FromAI(),
                target.Center + sourcePos,
                velocity, 
                ModContent.ProjectileType<DragonSkullFlames_Proj>(), 
                damage, 
                4, 
                Player.whoAmI
            );
